...
// AikoKey is the key type of Instance in Context, exported for test.
type AikoKey int

const aikoKey AikoKey = 1

// FromContext returns an Instance from the given context, or nil if the context doesn't contain one.
func FromContext(ctx context.Context) *Instance {
	if s, ok := ctx.Value(aikoKey).(*Instance); ok {
		return s
	}
	return nil
}
...
It is unsupported to use this function to create a context that is suitable to invoke Aiko's internal component
...
For third party code, usage enabled by creating a context to interact with Aiko's internal component is unsupported,
and may break at any time.
*/
func toContext(ctx context.Context, v *Instance) context.Context {
	if FromContext(ctx) != v {
		ctx = context.WithValue(ctx, aikoKey, v)
	}
	return ctx
}
